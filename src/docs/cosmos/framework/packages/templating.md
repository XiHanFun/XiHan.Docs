...
- **模板上下文**：`ITemplateContext` 承载变量/函数/作用域，Scriban 与 String 引擎都基于它取值。
...
- **安全/继承（进阶）**：`ITemplateSecurityAnalyzer` / `ITemplateSecurityChecker` 做模板安全分析；`ITemplateInheritanceManager` / `ITemplatePartialManager` 支持模板继承与片段复用。
...
| `DefaultTemplateEngine` | `ITemplateEngine<string>`，轻量替换 + 条件/循环 |
...
- **文件渲染**：`RenderFileAsync` 找不到文件会抛 `FileNotFoundException`。
