...
XiHan.Framework.Analyzers 是一套基于 Roslyn 的代码分析器与代码修复器，在**编译期**扫描 C# 源码是否符合框架规范，并在 IDE 中给出诊断提示和 Code Fix 一键修复。当前落地的规则是文件头版权版本注释检查（`XHFH001`）。它以 `netstandard2.0` 构建（Roslyn 分析器约定的目标框架），标记为开发期依赖（`DevelopmentDependency`），并按 analyzer 约定打包到 `analyzers/dotnet/cs`——因此只在开发/构建阶段生效，**不进入运行时**、不给消费方带来运行时依赖。
...
- **校验规则**：文件头必须严格匹配 `#region <<版权版本注释>>` … `#endregion` 区块，包含 MIT 授权声明、`FileName`、`Guid`、固定的 `Author:zhaifanhua` / `Email:me@zhaifanhua.com`、`CreateTime`（格式 `yyyy/MM/dd HH:mm:ss`）；且 `FileName` 必须与实际文件名一致、`Guid` 与 `CreateTime` 必须可解析。任一不满足即报 `XHFH001`。
- **修复行为**：Code Fix 会复用文件中已有的 `Guid` 与 `CreateTime`（若存在且可解析），否则生成新 `Guid`、取当前时间，再补写或替换成标准文件头。

## 规则

| 规则 ID | 标题 | 类别 | 默认级别 | 说明 |
| --- | --- | --- | --- | --- |
| `XHFH001` | 缺少曦寒标准版权文件头 | `XiHan.FileHeader` | `Warning`（默认启用） | C# 源文件必须包含曦寒标准版权版本注释，含 MIT 授权、作者、邮箱、文件名、GUID 与创建时间；缺失或不合规即触发，消息为「文件 '{0}' 缺少或未正确声明曦寒标准版权文件头」。 |
...
`XHFH001` 校验并由 Code Fix 生成的文件头形如（`FileName` 需与文件名一致，`Guid` / `CreateTime` 需合法）：
...
// CreateTime:2025/03/17 13:56:10
// ----------------------------------------------------------------
...
- 校验是**严格匹配**：`Author`/`Email` 为固定值，`FileName` 必须与实际文件名一致，`CreateTime` 必须是 `yyyy/MM/dd HH:mm:ss` 格式，行尾会归一化后再比对。
