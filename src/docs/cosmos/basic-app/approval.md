...
| 用户主动申请某个权限，管理员审批 | **权限申请**（`SysPermissionRequest`，见 [权限模型](./permissions)） |

它们可以组合：约束规则的 `RequireApproval` 就是把违规转成审批单；工作流的 `UserTask` 也可以承载复杂的权限审批流。

## 相关页面

- [权限模型](./permissions)：约束规则在鉴权判定链里的位置、权限申请与委托
- [工作流](./workflow)：多步骤流程
- [审计日志](./audit-log)：权限变更留痕
- [数据模型](./architecture/data-model#权限与授权)：相关表结构
