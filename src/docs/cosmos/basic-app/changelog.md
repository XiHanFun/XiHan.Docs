...
- **调整** 破坏性变更：动态 API 路径参数全面改走 query，前端 117 处路径拼接改为查询参数并移除 `formatDynamicApiRouteValue`
- **调整** 升级通道改为台账驱动，废除 version.txt 状态机，执行权改用数据库建议锁，脚本失败即阻止启动
- **调整** 升级通道纳入租户独立库，独立库租户各自持台账并补齐到当前程序版本
- **调整** 各模块自持 PageRegistry，菜单种子统一按 MenuCode 幂等落库，页面缓存默认开启，标签页切走再切回不再丢失表单内容与滚动位置
- **调整** 一级菜单按功能域重排，新增「AI 应用」与「功能配置」目录，日志独立为「审计中心」
- **调整** 菜单名统一：身份与权限改为身份权限、租户改为租户管理、AI 改为 AI 应用，审计中心下的数据变更、权限变更、链路追踪补全为「日志」命名
- **调整** MCP Server 改用框架 Web.Mcp 模块，应用层不再直接引用协议包
- **修复** 补齐 10 个端点漏标的授权特性，服务监控端点不再对任意登录用户开放宿主机硬件与依赖清单
- **修复** 模块权限链种子改在平台租户上下文内播种，补回干净库重建后缺失的 7 个菜单
- **修复** 修复升级通道读取租户时启动崩溃
- **修复** 修复连接测试、同步表结构、重置应用密钥三处因路由与后端不一致导致的 404
- **修复** 表格局部全屏在暗色模式下底色为白
- **升级** 升级依赖，发布 v3.10.0
