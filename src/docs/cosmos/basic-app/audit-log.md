...
所有六张表都是**只追加、永不更新**的审计数据，并且都是**按月分表**（`Sys_Xxx_Log_{year}{month}{day}`，`SplitType.Month`）。这带来一条硬约束：**查询与清理必须带时间范围**，否则框架会扫描全部月表。业务上不支持删除，按合规保留期 `TRUNCATE`/`DROP` 历史月表。
...
开放接口层的逐请求明细，字段最全：`RequestParams`/`RequestBody`/`ResponseBody`（均限长 32000 字符）、`RequestSize`/`ResponseSize`、`ExecutionTime`、`IsSignatureValid`/`SignatureType`（签名算法枚举：`HmacSha256`/`HmacSha512`/`RsaSha256`/`RsaSha512`/`Sm2`/`Md5`/`None`）、`ClientId`/`AppId`（第三方接入方）。
...
还落有 `ThreadId`/`ProcessId`/`ServerHostName`/`EnvironmentName`/`ApplicationVersion` 等运行环境快照，便于生产复现。字段 `IsHandled`/`HandledTime`/`HandledBy`/`HandledRemark` 支持异常"处理闭环"（谁在何时标记已处理）。
...
`LoginResult` 枚举既覆盖登录结果（`Success`/`InvalidCredentials`/`AccountLocked`/`AccountDisabled`/`RequiresTwoFactor`/`TwoFactorFailed`/`Failed`），也覆盖**认证审计事件**（`Logout`/`TokenRefreshed`/`PasswordChanged`/`PasswordReset`/`MfaBound`/`MfaUnbound`）——即令牌刷新、改密、绑定/解绑 MFA 也统一落这张表。`IsRiskLogin` 由风控规则判定（异地/新设备/深夜登录），登录事件处理器还会顺带做**并发登录检测**并推送安全通知（新设备/其它设备登录提醒）。
...
变更类型区分 **新增 / 修改 / 删除 / 恢复**，复用 `OperationType` 枚举（`Create`/`Update`/`Delete`/`Restore`）。关键判定：**软删除**是 `IsDeleted` 从 false→true 的 `Update`，框架识别后记为 `Delete`；软删除**还原**记为 `Restore`。写入器（`SaasEntityDiffLogWriter`）还按操作类型自动定级 `RiskLevel`（`AuditRiskLevel`：低/中/高/极高/严重）——删除、导出、执行命令定为极高（4），修改/恢复/审核/审批/导入定为高或中，查询定为低。

## 自动脱敏

敏感数据在**落库前**、于请求捕获点由框架的 `LogSanitizer`（`XiHan.Framework.Web.Api.Logging.LogSanitizer`）统一脱敏——写入器拿到的记录已经是脱敏后的副本，脱敏后的日志与原始请求缓冲区互不影响。脱敏覆盖三种形态：
...
访问 / API / 操作 / 异常 / 登录五类走框架的 `Pipeline` 抽象，**默认同步**写入（在请求线程内直接 `await writer.WriteAsync`）；可通过配置节 `XiHan:Web:Api:LogQueue` 切换为**异步队列**模式：
...
菜单由后端 `PageRegistry` 单一注册（建菜单即绑权限码），六个页面挂在"日志"分组下：
...
- **日志管道与脱敏**：中间件/过滤器/队列/`LogSanitizer` 由框架 `XiHan.Framework.Web.Api.Logging` 提供，BasicApp 只提供 Writer 落库实现。
- **实体变更拦截**：由框架 `XiHan.Framework.Data.Auditing` 的 SqlSugar AOP 提供，BasicApp 提供 `IEntityDiffLogWriter` 与 `IEntityAuditContextProvider`。
