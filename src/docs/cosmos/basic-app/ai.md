...
BasicApp 的 AI 能力落在独立模块 `XiHan.BasicApp.AI` 里，与代码生成模块同构（独立工程、独立种子段、独立权限资源）。它坐落在框架薄层 [XiHan.Framework.AI](../framework/packages/ai) 之上：**框架**提供 provider 解析、会话/嵌入门面、RAG 切片检索、Agent 与 MCP 桥接的抽象与默认实现；**BasicApp** 在其上做两件业务落地——把 AI Provider 配置**库化管理**（存库 + 加密 + 前端可维护 + 热切换），以及把 RAG 知识库**接上真实向量库（Qdrant）**并暴露为文档管理 / 检索问答 / MCP 工具。
...
模块前端页面挂在「开发工具（develop）」目录下：**AI 提供商**（`/develop/aiProvider`）与**知识库**（`/develop/knowledge`），二者均仅对持有 `ai:read` / `knowledge_base:read` 者（默认=超管通配 `*`）可见。
...
3. **重建数据库**：BasicApp 采用重建库策略（不做向后兼容），首次启动自动建表并跑种子；AI 相关表 `Sys_Ai_Provider` / `Sys_Knowledge_Document` 随之建立。
...
种子说明：AI provider 与知识库各占独立种子 Order 段（`200-204` / `205-208`，晚于 Saas 的 `10-37` 与代码生成的 `100-105`），链内顺序均为「操作字典 → 资源 → 权限 → 菜单 → 角色授权」，权限**仅授超管**（其余角色/租户按需在权限管理里下放）。

---

## 相关文档

- [框架 · XiHan.Framework.AI](../framework/packages/ai)：底层 provider 解析/热切换、RAG 切片检索、Agent/MCP 桥接的抽象与默认实现
